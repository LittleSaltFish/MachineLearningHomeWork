...
def TrainAndTest(trainset, testset):
    ...
    results = clf.predict([[x[0], x[1]] for x in testset])
...
def DrawAndSaveData(set_results, support, name, dir):
    ...
    for i in range(0, len(set_results)):
        if i in support and set_results[i][2] == 1:
            draw_s1.append(
                [set_results[i][0], set_results[i][1], set_results[i][2]])
...
def main():
    ...
    set_results, support_vectors, support, n_support = TrainAndTest(
        trainset, testset)
    DrawAndSaveData(set_results, support, "SVM results",
                    '../Data&Results/Classify/')
    print(set_results)
